...
def rgb_to_ycbcr(image):
    ...
    La conversión se realiza usando la función cvtColor de OpenCV.
    """
...
def downsample(image_ycbcr):
    ...
    El submuestreo 4:2:0 reduce la resolución de los componentes Cb y Cr 
    para lograr una compresión de imagen.
    """
    Y, Cb, Cr = cv2.split(image_ycbcr)
    
    # Reducir la resolución de Cb y Cr mediante el uso de pirámides gaussianas.
    # Esto equivale a submuestreo 4:2:0 donde se disminuye la resolución 
...
def split_into_blocks(channel):
    ...
    Esto es útil para aplicar la Transformada Discreta del Coseno (DCT) 
    en bloques pequeños como se hace en la compresión JPEG.
    """
    ...
    for i in range(0, height, 8):
        for j in range(0, width, 8):
            ...
            # Asegurarse de que el bloque extraído tenga el tamaño correcto (8x8).
            if block.shape == (8, 8):
                blocks.append(block)
...
def dct_2d(block):
    ...
    La DCT transforma el bloque espacial en el dominio de la frecuencia.
    """
    # Restar 128 para centrar los valores alrededor de 0 y aplicar la DCT.
...
def quantize(block, quant_matrix):
    ...
    La cuantización reduce la precisión de los coeficientes DCT según la matriz.
    """
...
def idct_2d(block):
    ...
    """
    # Aplicar la IDCT y sumar 128 para volver a la gama de valores originales.
...
def combine_blocks(blocks, height, width):
    ...
    """
    # Calcular el número de bloques necesarios en las dimensiones vertical y horizontal.
    num_blocks_y = height // 8
    num_blocks_x = width // 8
    
    # Inicializar una imagen en blanco con las dimensiones deseadas.
    image = np.zeros((height, width), dtype=np.float32)
    
    block_idx = 0
    # Iterar sobre la imagen para colocar los bloques en la imagen final.
    for i in range(0, height, 8):
        for j in range(0, width, 8):
...
def save_as_jpeg(y, cb, cr, output_filename):
    ...
    La imagen YCbCr se convierte primero a RGB antes de guardar.
    """
    # Combinar los canales Y, Cb y Cr en una imagen YCbCr.
    ycbcr = cv2.merge([y, cb, cr])
    # Convertir la imagen YCbCr a RGB.
    rgb = cv2.cvtColor(np.uint8(ycbcr), cv2.COLOR_YCrCb2RGB)
    # Crear una imagen PIL a partir de la matriz RGB y guardarla como archivo JPEG.
...
def pad_image(image):
    ...
    Esto es necesario para asegurarse de que la imagen pueda ser dividida en 
    bloques de 8x8 sin problemas.
    """
    h, w = image.shape[:2]
    h_pad = (8 - (h % 8)) % 8
    w_pad = (8 - (w % 8)) % 8
    
    # Añadir padding replicando los bordes de la imagen original.
...
# Definir las matrices de cuantización estándar para luminancia y crominancia
...
# Cuantizar los bloques de coeficientes DCT usando las matrices de cuantización
...
# Realizar la cuantización inversa para recuperar los coeficientes DCT
...
# Aplicar la Transformada Inversa del Coseno Discreto (IDCT) a los bloques cuantizados
...
# Combinar los bloques procesados en imágenes completas
Y_combined = combine_blocks(Y_idct, Y.shape[0], Y.shape[1])
Cb_combined = cv2.pyrUp(combine_blocks(Cb_idct, Cb.shape[0], Cb.shape[1]))
Cr_combined = cv2.pyrUp(combine_blocks(Cr_idct, Cr.shape[0], Cr.shape[1]))

# Guardar la imagen reconstruida como archivo JPEG
